...
uint8_t cache_invalidate_LLC(CACHE* LLC, uint64_t addr, uint32_t set, uint32_t way, uint32_t fill_cpu, uint64_t instr_id, uint64_t cycle){
  assert(LLC->cache_type == IS_LLC);
  
  PACKET wb;
  uint8_t dirty = 0; 

  for (int i=0;i<NUM_CPUS;i++){
    CACHE* L2 = LLC->upper_dcache[i];
    CACHE* L1I = L2->upper_icache[i];
    CACHE* L1D = L2->upper_dcache[i];
    CACHE* updated;
    uint32_t l1_set, l1_way;
    uint32_t l2_set, l2_way;
    uint8_t l1_hit = 0;

    // checking if address present in l1d
    l1_set = L1D->get_set(addr);
    l1_way = L1D->get_way(addr, l1_set);
    updated = L1D;
    if(l1_way < L1D_WAY) l1_hit = 1;   // L1D hit

    if(!l1_hit){
      l1_set = L1I->get_set(addr);
      l1_way = L1I->get_way(addr, l1_set);
      updated = L1I;
      if(l1_way < L1I_WAY) l1_hit = 1;        // L1I hit
    }

    if(l1_hit && updated->block[l1_set][l1_way].dirty){
      assert(!dirty);
      dirty = 1;
      wb.fill_level = FILL_DRC;  // to confirm yet
      wb.cpu = fill_cpu;
      wb.address = updated->block[l1_set][l1_way].address;
      wb.full_addr = updated->block[l1_set][l1_way].full_addr;
      wb.data = updated->block[l1_set][l1_way].data;
      wb.instr_id = instr_id;
      wb.ip = 0; // writeback does not have ip
      wb.type = WRITEBACK;
      wb.event_cycle = cycle;
    ...
    l2_set = L2->get_set(addr);
    l2_way = L2->get_way(addr, l2_set);
    if(l1_hit) assert(l2_way < L2C_WAY);
    if(l2_way < L2C_WAY){
      if(!dirty && L2->block[l2_set][l2_way].dirty){
        dirty = 1;
        wb.fill_level = FILL_DRC;  // to confirm yet
        wb.cpu = fill_cpu;
        wb.address = L2->block[l2_set][l2_way].address;
        wb.full_addr = L2->block[l2_set][l2_way].full_addr;
        wb.data = L2->block[l2_set][l2_way].data;
        wb.instr_id = instr_id;
        wb.ip = 0; // writeback does not have ip
        wb.type = WRITEBACK;
        wb.event_cycle = cycle;
  ...
  if(!dirty && LLC->block[set][way].dirty){
    dirty = 1;
    wb.fill_level = FILL_DRC;  // to confirm yet
    wb.cpu = fill_cpu;
    wb.address = LLC->block[set][way].address;
    wb.full_addr = LLC->block[set][way].full_addr;
    wb.data = LLC->block[set][way].data;
    wb.instr_id = instr_id;
    wb.ip = 0; // writeback does not have ip
    wb.type = WRITEBACK;
    wb.event_cycle = cycle;
  ...
  if(dirty && LLC->lower_level->get_occupancy(2, LLC->block[set][way].address) == LLC->lower_level->get_size(2, LLC->block[set][way].address)){
    return 0;
  }

  if(dirty){
    LLC->lower_level->add_wq(&wb);
  }
  ...
  for (int i=0;i<NUM_CPUS;i++){
    ...
    if(l1_hit){
      updated->block[l1_set][l1_way].dirty = 0;
      updated->block[l1_set][l1_way].valid = 0;
    }
    ...
    if(l1_hit) assert(l2_way < L2C_WAY);
    if(l2_way < L2C_WAY){
      L2->block[l2_set][l2_way].dirty = 0;
      L2->block[l2_set][l2_way].valid = 0;
    }
    
  }  
  LLC->block[set][way].dirty=0;
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
            way = LLC_WAY;//llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
        }
        else
        ...
        if (block[set][way].dirty || cache_type == IS_L2C) {
            ...
            if(cache_type == IS_L2C){
              do_fill= cache_invalidate_L2(this, block[set][way].address, set, way, fill_cpu, MSHR.entry[mshr_index].instr_id, current_core_cycle[fill_cpu]);
              if(!do_fill){
            ...
            //   do_fill= cache_invalidate_LLC(this, block[set][way].address, set, way, fill_cpu, MSHR.entry[mshr_index].instr_id, current_core_cycle[fill_cpu]);
            //   if(!do_fill){
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        //if(cache_type == IS_LLC) assert(way < 0); // LLC should not have already what was there in L2;
		...
		if(mshr_index == -2)
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                if (block[set][way].dirty || cache_type == IS_L2C) {
                    ...
                    // check if the lower level WQ has enough room to keep this writeback request
                    if(cache_type == IS_L2C){
                        do_fill= cache_invalidate_L2(this, block[set][way].address, set, way, writeback_cpu, WQ.entry[index].instr_id, current_core_cycle[writeback_cpu]);
                        if(!do_fill){
		  ...
		  if (cache_type == IS_L1I)
			...
			cpu = 0;
                    ...
                    // mark dirty
                    block[set][way].dirty = 1;
                    if(WQ.entry[index].copyback) block[set][way].dirty = 0;
